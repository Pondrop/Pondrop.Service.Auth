...
namespace Pondrop.Service.Auth.Infrastructure.CosmosDb;
...
public class CheckpointRepository<T> : ICheckpointRepository<T> where T : EventEntity, new()
{
    ...
    public async Task<bool> IsConnectedAsync()
    {
        ...
        try
        {
            ...
            if (_container is not null)
            {
                var spDirInfo = new DirectoryInfo(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, $"CosmosDb/AuthdProcedures/Checkpoint"));
                await _container.EnsureAuthProcedures(spDirInfo);
            }
        }
        catch (Exception ex)
        {
